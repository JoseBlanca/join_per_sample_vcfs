...
/// Site `QUAL = −10·log10 P(monomorphic | data)` from the freebayes-style cohort
/// marginal. `data_ll[s][g]` is the per-sample per-genotype read log-likelihood
/// (`em::compute_data_ll`), `pi` the EM cohort allele frequencies (to rank alleles),
/// `f_per_present` the frozen per-sample inbreeding `F`, `k` the candidate count, and
/// `theta` the SFS `θ`. Returns `0.0` (uninformative → monomorphic) for `< 2` alleles
/// or no present samples. **Not clamped** — the caller applies `qual_cap`.
///
...
pub(crate) fn site_qual(
    ...
    // `ln_p_mono ≤ 0` ⇒ QUAL ≥ 0. −10·log10(P) = −10·ln(P)/ln(10).
