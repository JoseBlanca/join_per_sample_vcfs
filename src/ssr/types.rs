...
/// left-aligned, minimally-trimmed form. Byte-equality **is** allele identity —
/// two samples carrying the same biological off-ladder allele must normalize to
/// identical bytes, or the cohort union would split one allele into several.
///
/// This type owns only the *invariant* that the bytes are already canonical; it
/// does **not** canonicalize. Constructing one is the caller's promise that the
/// bytes came from the shared sequence normalizer (the lifted SNP
/// `normalize_alleles` kernel), so equal alleles compare equal. `Eq`/`Hash`
/// therefore treat the bytes as opaque.
#[derive(Clone, PartialEq, Eq, Hash)]
pub(crate) struct NormalizedSeq(Box<[u8]>);

impl NormalizedSeq {
    /// Wrap bytes that are **already** in canonical normalized form.
    ///
    /// Canonicalization is not performed here (see the type doc): the caller
    /// guarantees `bytes` is the output of the shared normalizer.
    pub fn new(bytes: impl Into<Box<[u8]>>) -> Self {
        Self(bytes.into())
    }

    /// The canonical tract bytes.
    #[inline]
    pub fn as_bytes(&self) -> &[u8] {
        &self.0
    }
...
impl fmt::Debug for NormalizedSeq {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // Bases are ASCII; render as text for readable test output / logs.
        match std::str::from_utf8(&self.0) {
            Ok(s) => write!(f, "NormalizedSeq({s:?})"),
            Err(_) => write!(f, "NormalizedSeq({:?})", &self.0),
        }
    }
}

/// A repeat allele **at a given locus** — two encodings of one tract sequence,
/// not two kinds of thing. Locus-relative: an `Allele` is meaningful (and
/// comparable) only within one locus's candidate set, so it does not carry its
/// locus — the candidate set supplies that context.
///
/// The enum's `==` *is* sequence identity. `OnLadder` is injective in `units`
/// (with the locus fixed, `units` ↔ tract sequence is a bijection), and an
/// off-ladder allele is a non-rung sequence by construction, so the two variants
/// partition sequence space disjointly and can never collide. `units` is thus a
/// fast `u16` identity compare for the common case, not a rival notion of
/// identity.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub(crate) enum Allele {
    /// A clean rung of the repeat ladder: the motif tiled `units` times. The
    /// count is the lossless rung coordinate (the absolute repeat count, *not*
    /// the signed Δ from the reference), so the same rung is byte-identical in
    /// every sample.
    OnLadder { units: u16 },

    /// A non-rung sequence (an in-frame count plus a stray base, a partial unit,
    /// a variable interruption), carried explicitly as its canonical normalized
    /// tract.
    OffLadder(NormalizedSeq),
}

impl Allele {
    /// This allele's **tract** sequence — the repeat bases only, without flanks
    /// (the candidate-haplotype builder wraps the locus flanks around it). Pure;
    /// reads `locus.motif()`, never the reference FASTA.
    ///
    /// `OnLadder { units }` is a clean tiling of the motif (`motif × units`);
    /// an imperfect locus's interruptions are represented by `OffLadder`
    /// candidates rather than by bending the rung, matching the Stage-1 rung
    /// definition `left_flank + motif×L + right_flank`. `OffLadder` returns its
    /// canonical bytes verbatim.
    pub fn to_sequence(&self, locus: &Locus) -> Vec<u8> {
        match self {
            Allele::OnLadder { units } => {
                let unit = locus.motif();
                let unit = unit.as_bytes();
                let mut seq = Vec::with_capacity(unit.len() * *units as usize);
                for _ in 0..*units {
                    seq.extend_from_slice(unit);
                }
                seq
            }
            Allele::OffLadder(seq) => seq.as_bytes().to_vec(),
        }
    }

    /// Derived repeat count: an exact integer for `OnLadder`, a **fractional**
    /// count for `OffLadder` (its tract length over the motif period — e.g. 7 bp
    /// of a period-2 motif is 3.5 copies). Feeds VCF `REPCN` (rounded) /
    /// `BPDIFFS`; never an identity key.
    pub fn repeat_count(&self, locus: &Locus) -> f64 {
        match self {
            Allele::OnLadder { units } => f64::from(*units),
            // `period()` is 1..=MAX_MOTIF_LEN by `Motif::new`, never zero.
            Allele::OffLadder(seq) => seq.as_bytes().len() as f64 / locus.period() as f64,
        }
    }
}
...
mod tests {
    ...
    // --- Allele / NormalizedSeq (Stage-1 allele representation) ----------------

    #[test]
    fn on_ladder_to_sequence_tiles_the_motif() {
        let locus = sample_locus(); // motif "CA", period 2
        assert_eq!(Allele::OnLadder { units: 3 }.to_sequence(&locus), b"CACACA");
        assert_eq!(Allele::OnLadder { units: 1 }.to_sequence(&locus), b"CA");
        // Zero units is a well-defined empty tract, not a panic.
        assert_eq!(Allele::OnLadder { units: 0 }.to_sequence(&locus), b"");
    }

    #[test]
    fn on_ladder_to_sequence_round_trips_to_units() {
        // to_sequence ∘ count is the identity: tiling `units` motifs and dividing
        // the byte length back by the period recovers `units` exactly.
        let locus = sample_locus();
        for units in [0u16, 1, 5, 12, 40] {
            let seq = Allele::OnLadder { units }.to_sequence(&locus);
            assert_eq!(seq.len() / locus.period(), units as usize);
        }
    }

    #[test]
    fn on_ladder_repeat_count_is_the_integer_units() {
        let locus = sample_locus();
        assert_eq!(Allele::OnLadder { units: 12 }.repeat_count(&locus), 12.0);
        assert_eq!(Allele::OnLadder { units: 0 }.repeat_count(&locus), 0.0);
    }

    #[test]
    fn off_ladder_to_sequence_returns_canonical_bytes_verbatim() {
        let locus = sample_locus();
        let allele = Allele::OffLadder(NormalizedSeq::new(b"CACACAC".to_vec()));
        assert_eq!(allele.to_sequence(&locus), b"CACACAC");
    }

    #[test]
    fn off_ladder_repeat_count_is_fractional() {
        let locus = sample_locus(); // period 2
        // 7 bp / period 2 = 3.5 copies — a non-integer count, by design.
        let allele = Allele::OffLadder(NormalizedSeq::new(b"CACACAC".to_vec()));
        assert_eq!(allele.repeat_count(&locus), 3.5);
    }

    #[test]
    fn allele_equality_is_sequence_identity() {
        // Same rung ⇒ equal; different rung ⇒ unequal.
        assert_eq!(
            Allele::OnLadder { units: 12 },
            Allele::OnLadder { units: 12 }
        );
        assert_ne!(
            Allele::OnLadder { units: 12 },
            Allele::OnLadder { units: 13 }
        );

        // Same canonical bytes ⇒ equal; different bytes ⇒ unequal.
        let a = Allele::OffLadder(NormalizedSeq::new(b"CACACAC".to_vec()));
        let b = Allele::OffLadder(NormalizedSeq::new(b"CACACAC".to_vec()));
        let c = Allele::OffLadder(NormalizedSeq::new(b"CACACA".to_vec()));
        assert_eq!(a, b);
        assert_ne!(a, c);

        // The two variants partition sequence space — they never collide, even
        // when the off-ladder bytes equal an on-ladder rung's tiling.
        assert_ne!(
            Allele::OnLadder { units: 3 },
            Allele::OffLadder(NormalizedSeq::new(b"CACACA".to_vec()))
        );
    }

    #[test]
    fn allele_dedups_in_a_hashset() {
        use std::collections::HashSet;
        let mut set = HashSet::new();
        set.insert(Allele::OnLadder { units: 12 });
        set.insert(Allele::OnLadder { units: 12 }); // duplicate rung collapses
        set.insert(Allele::OffLadder(NormalizedSeq::new(b"CACACAC".to_vec())));
        set.insert(Allele::OffLadder(NormalizedSeq::new(b"CACACAC".to_vec()))); // dup
        assert_eq!(set.len(), 2);
        assert!(set.contains(&Allele::OnLadder { units: 12 }));
    }

    #[test]
    fn normalized_seq_exposes_its_bytes_and_renders_ascii() {
        let seq = NormalizedSeq::new(b"CACA".to_vec());
        assert_eq!(seq.as_bytes(), b"CACA");
        assert_eq!(format!("{seq:?}"), "NormalizedSeq(\"CACA\")");
    }
}
