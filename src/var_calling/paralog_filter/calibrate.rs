...
fn inbreeding_by_sample(prepass: &ParalogPrePass, hexp: f64) -> Vec<f64> {
    prepass
        .samples()
        .iter()
        .map(|s| match s {
...
mod tests {
    ...
    use crate::var_calling::paralog_filter::spill::ParalogSpillWriter;
    ...
    /// A single-copy coverage histogram (scale ≈ 20) every sample in the test
    /// cohort shares, so `relative_copy_number(gc, depth) ≈ depth / 20`.
    fn single_copy_summary() -> SampleSummary {
        let gc_bins = 4u32;
        ...
        // A tight single-copy peak at depth ~20 across all GC bins.
    ...
    fn prepass(n_samples: usize) -> ParalogPrePass {
    ...
    /// Build a biallelic-SNP spill record for `n_samples`, each sample given a
    /// window mean depth and an (ref_obs, alt_obs) AD.
    fn snp_spill(
        pos: u32,
        n_samples: usize,
        gc: f32,
    ...
    /// A "normal" locus: every sample at ~1× coverage (depth ≈ scale) with a
    /// clean het (balanced AD) — a real single-copy variant, LR should be < 0.
    fn normal_locus(pos: u32, n: usize) -> ParalogSpillRecord {
        snp_spill(pos, n, 0.5, &|_| (20.0, 10, 10))
    }

    /// A "paralog-like" locus: every sample at ~2× coverage with a skewed VAF
    /// (~1/3, a collapsed-paralog PSV), LR should be > 0.
    fn paralog_locus(pos: u32, n: usize) -> ParalogSpillRecord {
        snp_spill(pos, n, 0.5, &|_| (40.0, 20, 10))
    }

    fn write_spill(records: &[ParalogSpillRecord]) -> Vec<u8> {
        let mut w = ParalogSpillWriter::new(Cursor::new(Vec::new()));
        for r in records {
            w.append(r).unwrap();
        }
