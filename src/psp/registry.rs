...
super::kind::column_key! {
    ...
    /// matching on this (via [`ColumnKey::from_tag`]); the `tag()` /
    /// `from_tag()` pairing is a compile-time bijection generated by
    /// the [`column_key!`](super::kind::column_key) macro from one
    /// variant↔tag list, so adding a variant forces an arm at every
    /// dispatch site and a forgotten tag cannot compile (M4/M5).
    ///
    /// This is **SNP-schema-private**: it lives here, not on the
    /// schema-agnostic [`ColumnDef`] (which carries only wire metadata,
    /// so it can host any kind's columns). The SSR schema has its own
    /// key enum (`registry_ssr::SsrColumnKey`); the container generic
    /// core never dispatches on either.
    ///
    ...
    pub enum ColumnKey {
        ...
        AlleleQSumLog = 0x11,
        AlleleFwdCount = 0x12,
        AllelePlacedLeftCount = 0x13,
        AllelePlacedStartCount = 0x14,
        AlleleMapqSum = 0x15,
        AlleleMapqSumSq = 0x16,
        AlleleChainIds = 0x22,
    }
}
