...
>
> - **Last completed task (2026-07-17):** **ng step 3 typed regions — MILESTONE E COMPLETE. ✅ STEP 3 IS DONE, at Checkpoint E.**
...
>   1887 lib tests green in **both profiles**; fmt/clippy clean; **nothing outside `src/ng/`** across the whole plan. **⏸ CHECKPOINT E — step 3 complete.** **Needs your call:** (1) **`Satellite` clips at a BED edge** — the spec was silent and the rule came from the type; worth ruling on, as the absorption rule was. (2) **Three of §3.1's five rejection columns are structurally zero** — not a bug, but a thing a reader of the counts must know. (3) Still open from Checkpoint A: a `proptest` property over `assert_agrees`, the strongest remaining lever on the port. **Next:** the plan's out-of-scope list — the pileup/generic join, the STR gatherer, bundle disposal, and spec §10's routing experiments (now runnable: every rule is a knob and the walk is done).
