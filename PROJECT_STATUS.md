...
- **Status:** Milestone E **E1 implemented** (2026-06-23, branch `ssr-cohort`). The prior-side outer loop (`F_i` + per-group level) is in; **E2 (FP control + full VCF semantics) next**, then the milestone-E review. Milestone D shipped (full pre-pass; M3). Milestones A+B+C shipped (checkpoint 1+2 signed off).
- **Milestone E (E1) code:** [inbreeding.rs](src/ssr/cohort/inbreeding.rs) — `run_cohort_em` outer loop: per-locus EM with per-sample `F` + per-group level, `reduce_f` (excess-homozygosity estimator via `FixedPointAccum`, shrink-to-mean, clamp ≤ 0.99), `reduce_level` (hard-attribution refit). `em::run_locus_em_with` (per-sample `F`, `level_per_group`, `posterior_hom` output) added; `run_locus_em` is now a wrapper. 1245 lib tests (+3).
