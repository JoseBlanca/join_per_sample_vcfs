...
- **Latest review:** [genotype_em_phase2_2_2026-07-07.md](doc/devel/reports/reviews/genotype_em_phase2_2_2026-07-07.md) — Approve-with-changes → fixes applied (M1 regression-test iteration-count pin; convergent naming/reliability doc fix; 2 coverage tests added). 2.3 review (refactor_safety): No findings.
- **Open:**
  - **Phase 3** (next major chunk): relocate the trait + generic loop to crate-level `src/genotype_em/` + split `RecordScratch` into shared-core vs per-model state (was Phase 2.4/2.5, deferred here to land at the real destination once); add the `finalize` hook; build `SsrModel` (mode-centred `G₀` seed + stutter read-model). SSR call-changing, benchmark-gated on ssr_tomato1-vs-HipSTR.
