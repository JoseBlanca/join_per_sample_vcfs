...
>
> - **Last completed task (2026-07-04):** **SFS-marginalized SNP genotype prior — implemented (Steps A/B0/B + engine wiring) and GIAB-validated; generalization designed**
>   (branch `sfs-genotype-prior`). Root-caused the single-sample low-coverage SNP het over-call (true hom-alt called het) to the posterior engine's plug-in HWE prior: for n=1 the `α_ref=10` Dirichlet pins `p̂_alt≈0.08`, so HWE skews ~22:1 toward het and overrides the weak low-depth likelihood. Fix = replace the plug-in with an **SFS-marginalized Wright HWE+F** genotype prior (het:hom-alt = `2(1−F)/(1+F)`, θ-independent). Landed types-first, each step multi-agent-reviewed + committed: **A** `DiversityEstimate` (F-free copy-count θ + per-sample F from `.psp` summaries, no new pileup pass); **B0** extracted shared Wright-HWE + SFS-grid primitives to `src/genetics.rs` (paralog byte-identical); **B** `SfsGenotypePrior`; wired via `e_step_sfs_biallelic` (biallelic-diploid; multiallelic/non-diploid keep HWE). **GIAB result: 5× SNP GT concordance 83.6 % → 94.6 %** (beats freebayes at every depth), `1/1→0/1` over-call 214 → 8, **precision/recall/FP byte-identical** (the prior only moves genotypes among emitted variants); indels unchanged at depth (AD-driven, separate). Report: [sfs_prior_giab_validation_2026-07-04.md](doc/devel/reports/sfs_prior_giab_validation_2026-07-04.md). Spec/arch/plan under `doc/devel/{specs,architecture,implementation_plans}/sfs_genotype_prior*.md`. **Next:** generalize the prior to all `(ploidy, n_alleles)` via the **Dirichlet-multinomial** closed form (α_alt tied to θ, F via the existing Wright mixture) and delete the HWE plug-in — designed in the arch doc §9 + [sfs_genotype_prior_generalization.md](doc/devel/implementation_plans/sfs_genotype_prior_generalization.md), to be done in a fresh session.
