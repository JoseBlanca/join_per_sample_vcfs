...
- **Status:** Milestone D **D1+D2 reviewed — ▶ CHECKPOINT 2 signed off** (2026-06-23, branch `ssr-cohort`). The pre-pass recovers the simulator's known `ε` / stutter shape / level. Fixes (loop step 5) next, then D3 (clustering + per-group shape + ε-freeze — M3). Milestones A+B+C shipped (checkpoint 1 signed off).
- **Milestone D (D1+D2) review:** [ssr_call_genotyping_milestone_d_d1d2_2026-06-23.md](doc/devel/reports/reviews/ssr_call_genotyping_milestone_d_d1d2_2026-06-23.md) — **Approve-with-changes**: 0 Blocker, 0 Major, 2 Minor + Nits + 2 missing tests (Mi1 het-ε bias doc; Mi2 unclamped level-line doc; het-contribution + determinism tests).
