...
- **Open:**
  - **Milestone D** — `ReadFilter` iterator + `new` (fail-fast contig validation) + `counts` + fixture test on a small **BAM** (and now a CRAM). **Carry:** enforce the `DropReason`↔`ReadFilterCounts` 1:1 mapping with an exhaustive `match` at the tally site (A/B); the reused-buffer no-stale assertion (D3); the fatal read-error test deferred from C (M2); split the two noodles adapters + traits into a `record_source` submodule (both sources now justify it). Note: hand-count against the **#7→#9→#8** order — per-bucket `bad_cigar`/`high_mismatch_fraction` differ from production for both-failing reads by the settled reorder (expected, not a parity break).
