...
- **Status:** Milestone D **D1+D2 implemented — ▶ CHECKPOINT 2 reached** (2026-06-23, branch `ssr-cohort`). The pre-pass recovers the simulator's known `ε` / stutter shape / level. **Loop halts here for human sign-off** before D3 (per the plan's checkpoint gate); milestone-D review is the next loop step. Milestones A+B+C shipped (checkpoint 1 signed off).
