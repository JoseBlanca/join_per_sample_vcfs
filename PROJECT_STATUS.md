...
- **Status:** Milestone C **reviewed — ▶ CHECKPOINT 1 signed off** (2026-06-23, branch `ssr-cohort`). End-to-end `ssr-call` on supplied params: called genotypes match the simulator's truth. Fixes (loop step 5) next, then Milestone D (the pre-pass → checkpoint 2). Milestones A+B shipped.
- **Milestone C review:** [ssr_call_genotyping_milestone_c_2026-06-23.md](doc/devel/reports/reviews/ssr_call_genotyping_milestone_c_2026-06-23.md) — **Approve-with-changes**: 0 Blocker, 0 Major, 2 Minor + Nit + 1 missing test (Mi1 ploidy-panic doc; Mi2 first-of-length doc; moderate-stutter EM test).
