...
>   Mi3/Mi6); **M3** added `ContainerCache` eviction/dedup unit tests; plus Mi1/Mi4/Mi5/Mi7/Mi8/Mi9/Mi10 + doc nits.
>   **M1/M4 reworded as docs** per the review discussion — first working version, **no `.ssr.psp` baseline/golden**
>   (verified), so H1/H2's within-ULP math difference is inert and the cache cap (non-output-affecting) stays a fixed
>   constant. All gates green: fmt/clippy `-D warnings`/doc clean, **1171 lib (+4 tests) + 45 integration** pass.
>   Deferred: M3 multi-container *fetch* test (fixture gap), Mi2/Mi11/Mi12. Audit trail
>   `tmp/review_2026-06-17_ssr-pileup-review/`.
> - **Prior task (2026-06-17):** **ssr-pileup code review**
>   (branch `ssr-pileup-review`, [ssr_pileup_2026-06-17.md](doc/devel/reports/reviews/ssr_pileup_2026-06-17.md),
>   11 categories). First *correctness* review of the perf branch (pair-HMM math rewrites + shared-prefix DP,
>   window 10→6, per-worker `CachingCramReader`). **Verdict: Request-changes** — 1 Blocker, 5 Major, 12 Minor + Nits.
>   Gates: fmt/clippy `-D warnings` clean, 1167 lib tests pass — but **`cargo doc` FAILS** (**B1**: fetch_reads.rs:9
>   links the removed `AlignmentFile`). Crux is **M1**: the "byte-identical to `main`" claim for the `ln_1p`/single-pass
>   math is asserted but never verified against `main` (the bit-identity test compares the new code to itself; a 2M-sample
>   probe shows 4.74% f64-bit divergence) — run `ssr_psp_concordance` with window pinned to 10 to settle it. Also M2
>   `decoded==0` walk divergence from `refill`, M3 cache-eviction untested, M4 cache-cap uninspectable, M5 walk/decode
>   duplication. P1 shared-prefix DP confirmed provably bit-identical; `map_init` concurrency sound. Audit trail
>   `tmp/review_2026-06-17_ssr-pileup-review/`. **(suggested follow-up: apply fixes — B1 first, then M1's concordance run.)**
> - **Prior task (2026-06-16):** **ssr-pileup performance review**
