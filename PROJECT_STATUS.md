...
>
> - **Last completed task (2026-07-20):** **ng `type-regions` CLI (`pop_var_caller_exp`) — MILESTONE B COMPLETE (the input surface), at Checkpoint B.** Plan-driven, one implement→review→apply→commit loop per step. The CLI now parses every knob at its short-read default; the driver body is still Milestone E.
