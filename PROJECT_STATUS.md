...
- **Status:** Milestone F **F1 implemented** (2026-06-23, branch `ssr-cohort`). Parallelism + byte-identity across thread counts done; F2 calibration = provisional `dev_default`s pending real data (correctness gates tested). Review (loop step 3) next. **All plan milestones A–F now implemented — the algorithmic SSR caller is complete and parallel.** Milestone E shipped (caller composes end-to-end); D shipped; A+B+C shipped (checkpoints 1+2 signed off).
