...
- **Status:** Milestone F **F1 reviewed** (2026-06-23, branch `ssr-cohort`). Parallelism + byte-identity across thread counts done; F2 calibration = provisional `dev_default`s pending real data (correctness gates tested). Fix (loop step 5) next. **All plan milestones A–F now implemented — the algorithmic SSR caller is complete and parallel.** Milestone E shipped (caller composes end-to-end); D shipped; A+B+C shipped (checkpoints 1+2 signed off).
- **Milestone F review:** [ssr_call_genotyping_milestone_f_2026-06-23.md](doc/devel/reports/reviews/ssr_call_genotyping_milestone_f_2026-06-23.md) — **Approve-with-changes**: 0 Blocker, 0 Major, 1 Minor (doc: `PrepassStats` struct order is not canonical — determinism holds for the derived params, tested). Concurrency review: no `unsafe`/locks/atomics; determinism structural.
