...
- **Status:** implemented (Milestones A+B+C of 4; A–D plan). Per-milestone review loop; no whole-feature review yet.
...
- **Code:** [src/ng/read/filtering.rs](src/ng/read/filtering.rs) (types + `verdict_*` cascade + the `RawRecord`/`RecordSource` seam + `NoodlesRawRecord`/`BamRecordSource`), [src/ng/read/mod.rs](src/ng/read/mod.rs), [src/ng/types.rs](src/ng/types.rs). Reuses `record_buf_to_mapped_read` ([src/bam/alignment_input.rs](src/bam/alignment_input.rs), visibility lifted `pub(super)`→`pub(crate)`).
- **Impl reports:** [A (types)](doc/devel/reports/implementations/ng_read_filtering_a_2026-07-14.md), [B (cascade)](doc/devel/reports/implementations/ng_read_filtering_b_2026-07-14.md), [C (seam)](doc/devel/reports/implementations/ng_read_filtering_c_2026-07-14.md)
- **Latest reviews:** [A](doc/devel/reports/reviews/ng_read_filtering_a_2026-07-14.md) (0/0/4), [B](doc/devel/reports/reviews/ng_read_filtering_b_2026-07-14.md) (0/1/6), [C](doc/devel/reports/reviews/ng_read_filtering_c_2026-07-14.md) (0 Bl/2 Maj[test-coverage]/6 Mi) — all Approve-with-changes.
- **Latest fixes-applied:** [A](doc/devel/reports/reviews/fixes_applied_2026-07-14.md), [B](doc/devel/reports/reviews/fixes_applied_2026-07-14_v2.md), [C](doc/devel/reports/reviews/fixes_applied_2026-07-14_v3.md).
...
- **Milestone C done (the record seam):** `RawRecord`/`RecordSource` traits + a test fake + `NoodlesRawRecord` (wraps a noodles `RecordBuf`; flag/MAPQ pre-decode reads, `decode` reuses `record_buf_to_mapped_read`) + `BamRecordSource<R>` (`read_next` via noodles `read_record_buf`, true buffer reuse). 24 tests incl. an in-memory-BAM round-trip. **Deviation:** `decode` is `io::Result` (the reused decoder is fallible; a decode Err is fatal like the #8 fetch). **Scope decision (surfaced at Checkpoint C): BAM only — the CRAM `RecordSource` is deferred** to a sibling impl (CRAM decodes at container granularity into owned records + needs a reference at decode time, so it does not fit the one-record-into-a-reused-buffer shape).
- **Open:**
  - **Milestone D** — `ReadFilter` iterator + `new` (fail-fast contig validation) + `counts` + fixture test on a small **BAM**. **Carry:** enforce the `DropReason`↔`ReadFilterCounts` 1:1 mapping with an exhaustive `match` at the tally site (A/B); the reused-buffer no-stale assertion (D3); the fatal read-error test deferred from C (M2); optionally split the noodles adapters into a `record_source` submodule. Note: hand-count against the **#7→#9→#8** order — per-bucket `bad_cigar`/`high_mismatch_fraction` differ from production for both-failing reads by the settled reorder (expected, not a parity break).
  - **CRAM `RecordSource`** — a tracked follow-up (container-buffer source yielding owned records; needs a reference `Repository`).
