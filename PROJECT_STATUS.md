...
- **Status:** Milestone C **implemented — ▶ CHECKPOINT 1 reached** (2026-06-23, branch `ssr-cohort`). End-to-end `ssr-call` on supplied params: called genotypes match the simulator's truth. **Loop halts here for human sign-off** before Milestone D (per the plan's checkpoint gate); milestone-C review is the next loop step. Milestones A+B shipped.
