...
- **Status:** Milestone E **shipped** (2026-06-23, branch `ssr-cohort`). E1 (prior-side `F`+level outer loop) + E2 (FP control + full VCF semantics) done, reviewed, fixed; the whole caller composes end-to-end (proven by an integration test). **Milestone F (scale & calibrate — parallelism + byte-identity, the last) next** in the loop. Milestone D shipped; A+B+C shipped (checkpoints 1+2 signed off).
