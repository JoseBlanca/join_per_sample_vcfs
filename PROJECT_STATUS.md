...
- **Perf (review + applied wins, 2026-06-16):**
  ...
  - **Open — fetch path (L7, profiled 2026-06-16):** the end-to-end wall on real data is **per-locus CRAM slice decode + reference-MD5 validation** (`CramSegmentReads::next`; MD5 is ~29% and noodles-hardcoded, only reducible by decoding each slice fewer times). Fix = coalesce adjacent loci into one query (decode each slice once). **Catch:** byte-identity needs the per-locus filter-drop tallies, so coalescing requires *moving the read filter out of the reader into the SSR fetch layer* (query union unfiltered, filter per-locus). Contained but non-trivial; designed in the investigation report, not yet implemented — recommended next step.
