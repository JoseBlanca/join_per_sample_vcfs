...
- **Status:** in progress on branch `tomato2-paralog-filter`. **P1 + Q1–Q5 + R1 done; Milestone S wiring in flight (S1–S2 done, S2 checkpoint reached).**
...
- **Carried forward:** downstream `Hobs` consumers (S1) must guard the zero-callable case (`validate()` admits `callable_positions == 0` for an all-`N` sample); Q4 handles it (zero rate → inert `F`). Next: **S3** (inline per-window `(gc, mean depth)` in the caller fold — hot-path, byte-identity-guarded), then S4–S6.
