...
- **Status:** Milestone E **reviewed** (2026-06-23, branch `ssr-cohort`). E1 (prior-side `F`+level outer loop) + E2 (FP control + full VCF semantics) done; fixes (loop step 5) next, then Milestone F (scale & calibrate — the last). Milestone D shipped (full pre-pass; M3); A+B+C shipped (checkpoints 1+2 signed off).
- **Milestone E review:** [ssr_call_genotyping_milestone_e_2026-06-23.md](doc/devel/reports/reviews/ssr_call_genotyping_milestone_e_2026-06-23.md) — **Approve-with-changes**: 0 Blocker, 0 Major, 2 Minor + Nits + 1 missing test (Mi1 `Δlevel` convergence; Mi2 `add_bin` dedup; end-to-end pipeline test).
