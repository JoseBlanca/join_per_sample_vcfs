...
- **Status:** Milestone D **D1+D2 shipped — ▶ CHECKPOINT 2 signed off** (2026-06-23, branch `ssr-cohort`). The pre-pass recovers the simulator's known `ε` / stutter shape / level. **D3 (clustering + per-(group,period) shape + ε-freeze — the M3 milestone) next** in the loop. Milestones A+B+C shipped (checkpoint 1 signed off).
