...
- **Status:** implemented (Milestones A+B of 4; A–D plan). Per-milestone review loop; no whole-feature review yet.
...
- **Code:** [src/ng/read/filtering.rs](src/ng/read/filtering.rs) (step-1-local types + the two-phase `verdict_*` cascade), [src/ng/read/mod.rs](src/ng/read/mod.rs), [src/ng/types.rs](src/ng/types.rs) (scalar newtypes + `DomainError`).
- **Impl reports:** [A (types)](doc/devel/reports/implementations/ng_read_filtering_a_2026-07-14.md), [B (cascade)](doc/devel/reports/implementations/ng_read_filtering_b_2026-07-14.md)
- **Latest reviews:** [A](doc/devel/reports/reviews/ng_read_filtering_a_2026-07-14.md) (0 Bl/0 Maj/4 Mi), [B](doc/devel/reports/reviews/ng_read_filtering_b_2026-07-14.md) (0 Bl/1 Maj[M1]/6 Mi) — both Approve-with-changes.
- **Latest fixes-applied:** [A](doc/devel/reports/reviews/fixes_applied_2026-07-14.md), [B](doc/devel/reports/reviews/fixes_applied_2026-07-14_v2.md).
...
- **Milestone B done (the cascade, pure):** `verdict_pre_decode` (#1–#6, order identical to production `classify_pre_decode`) + `verdict_post_decode`, unit-tested against `InMemoryRefSeq` (18 tests). **Post-decode order settled with the owner as too-short (#7) → bad-CIGAR (#9) → high-mismatch (#8)** — cheapest/no-reference first, the one reference-touching filter last (spec §3's own "cheapest-first" principle). This *reorders* the spec's #7/#8/#9 table: keep/drop set unchanged, a both-failing read charged to `BadCigar` (root cause) not `HighMismatchFraction`. #8 runs on the original (un-left-aligned) CIGAR (left-alignment deferred to `pileup/`, spec §6; the mismatch tally is invariant under legal left-shifts). **M1 error model settled with the owner: fatal** — an `OutOfBounds` #8 fetch (read window past contig end) aborts the run (malformed record), it does not skip-and-keep as production does.
- **Open:**
  - **Milestone C** — `RawRecord`/`RecordSource` traits + test fake + ng-owned noodles adapter (flag/MAPQ pre-decode + `RecordBuf → MappedRead` decode).
  - **Milestone D** — `ReadFilter` iterator + `new` (fail-fast contig validation) + `counts` + fixture drop-parity test. **Carry (from A/B):** enforce the `DropReason`↔`ReadFilterCounts` 1:1 mapping with an exhaustive `match` at the tally site; add the reused-buffer no-stale assertion (D3). Note: the drop-parity fixture must hand-count against the **#7→#9→#8** order (not the spec table), and per-bucket `bad_cigar`/`high_mismatch_fraction` counts will differ from production for both-failing reads (by the settled reorder) — that is expected, not a parity break.
