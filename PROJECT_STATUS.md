...
>   **Next:** **Stage-2 stutter scoring** — confirm `ssr-call` scores in-frame (unit) vs out-of-frame (non-unit) length differences distinctly (HipSTR `log_stutter_pmf`) and reconcile the delimiter's provisional `GAP_OPEN_PROB_TRACT` with that slippage rate; answer review Q1 (is Stage-1 `.ssr.psp` output sparse?); real-data calibration; the first real cohort SSR call.
