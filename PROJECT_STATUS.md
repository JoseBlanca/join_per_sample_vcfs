...
- **Open (tracked follow-ups):**
  - **`finish` observability residual** — because the item is a bare `MappedRead` (spec §5), a `for`-loop that never calls `finish` still loses a latched fatal error silently in *release* (Drop debug-assert + `#[must_use]` + docs mitigate dev/test). Inherent to the spec's surface; the real pipeline consumer must call `finish`. **Surfaced to the owner** — a stronger guarantee would revisit the `Item = MappedRead` decision.
