...
- **Status:** Milestones A+B+C **shipped — ▶ CHECKPOINT 1 signed off** (2026-06-23, branch `ssr-cohort`). End-to-end `ssr-call` on supplied params calls genotypes matching the simulator's truth. **Milestone D (the parameter pre-pass → checkpoint 2) next** in the loop.
