...
- **Phase 3 (lightweight, in-flight):** give SSR the improved marginalize+LOO DM prior seeded by its own mode-centred `G₀`, sharing `genetics::dirichlet_multinomial_log_priors`; **SNP E-step untouched** (protects the SIMD gains); two slim loops stay separate. **Phase 3.2 done** — the SSR prior wrapper (`diploid_dm_inputs`, `leave_one_out_alpha`, `marginalized_genotype_log_priors`) added to [em.rs](src/ssr/cohort/em.rs), additive + unit-tested (math verified identical to the SNP mixture), unwired. Design: architecture doc §7, plan Phase 3.
- **Latest reviews:** [genotype_em_phase2_2_2026-07-07.md](doc/devel/reports/reviews/genotype_em_phase2_2_2026-07-07.md) (Phase 2.2); [ssr_marginalized_prior_phase3_2_2026-07-07.md](doc/devel/reports/reviews/ssr_marginalized_prior_phase3_2_2026-07-07.md) (Phase 3.2 — Approve-with-changes → fixes applied: M1 intermediate-`f` + `f=1`-value tests pin the IBD normalization; shape/boundary tests; out-of-range `debug_assert`; `diploid_dm_shape`→`diploid_dm_inputs` rename).
- **Open:**
  - **Phase 3.3** — small shared convergence-discipline helper (test the driver `expected_counts`); **3.4** — wire the new prior into `run_pi_em` behind an `EmCfg` toggle (default = plug-in, SSR byte-identical); **3.5** — benchmark ssr_tomato1-vs-HipSTR (watch `G₀`-as-DM-concentration), decide default.
