...
>   (branch `ssr-cohort`, impl `46df902`). The genotype EM now adapts the stutter **shape**
>   per locus (the deferred HipSTR-style adaptation): genotype under the frozen `θ_period`
>   seed → attribute reads to their called alleles → `refine_theta_locus` shrunk toward the
>   seed → re-genotype until settled. Local to each locus, so the sweep stays single-pass and
>   byte-identical. Reviewed (Approve-with-changes; 0 Blocker/Major, 2 Minor) + fixes applied
>   ([review](doc/devel/reports/reviews/ssr_call_theta_locus_2026-06-24.md),
>   [fixes](doc/devel/reports/reviews/fixes_applied_2026-06-24_theta_locus.md)). 1276 lib tests.
>   **Next (loop):** Step I2 (per-locus stutter **level**/rate refit, shrunk), then Step J (parallel sweep + byte-identity). Real-data calibration is the separate downstream effort.
...
- **Open:** Step **I2** (per-locus stutter **level**/rate refit, shrunk) and Step **J** (parallel sweep + byte-identity) unstarted. Calibration carry-overs: H4-Mi1 representative burn-in subset selection (positional first-cap biases chemistry **and** couples to the decision-E check); H1-Mi1 `G₀` backfill universe; filtered-record e2e test; Mi2 π warm-start across θ rounds (→ J throughput); all `dev_default` constants incl. `BURN_IN_MAX_LOCI` + the new `theta_*`.
