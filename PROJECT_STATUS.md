...
- **Status:** Milestone E **implemented** (2026-06-23, branch `ssr-cohort`). E1 (prior-side `F`+level outer loop) + E2 (FP control + full VCF semantics) done; review (loop step 3) next, then Milestone F (scale & calibrate — the last). Milestone D shipped (full pre-pass; M3); A+B+C shipped (checkpoints 1+2 signed off).
