...
>
> - **Last completed task (2026-06-01):** **Code review of the whole
>   `src/var_calling/` subtree** (Stages 3–6 + the `from_psp/` chunk
>   driver; 23 files, 27 489 LoC, commit `3c9ebf2`) —
...
>   gate M5/M8/M2/M9. `fmt`/`clippy` clean; 404 `var_calling` lib tests
>   pass; `cargo doc` red. Per-category audit trail at
>   `tmp/review_2026-06-01_var_calling/`. Suggested follow-up: apply fixes
>   (start with M1 + M3/M4/M7, the cheap CI/debris wins).
...
#### Within-chromosome chunk-parallel rewrite (`from_psp/`, formerly `cohort_block/`)
- **Status:** reviewed (2026-06-01, full-but-prioritized subtree review); fixes-applied (Wave 1); parallel block-consume shipped
...
- **Code:** [src/var_calling/from_psp/](src/var_calling/from_psp/) (renamed from `cohort_block/` in the `var_calling` reorg) — `mod.rs`, `columns.rs`, `column_span_reader.rs`, `loader.rs`, `partition.rs`, `driver.rs`, `worker.rs`, `test_helpers.rs`, plus `kernels/{mod, unify_alleles, project_scalars, compute_log_likelihoods}.rs`. (The streaming-columnar rewrite folded `pre_pass.rs` into the loader/partition path.)
