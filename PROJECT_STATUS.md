...
> - **Last completed task (2026-06-24):** **SSR Stage 2 (`ssr-call`) driver wiring — re-review fixes applied (B1 + all 6 Majors + 9 Minors)**
...
>   **Next:** answer review Q1 (is Stage-1 `.ssr.psp` output sparse?); pick up the deferred Minors (Mi1–Mi5, Mi13, Mi14, Mi16) and the SSR bench that gates Mi3/Mi4; then real-data calibration of the provisional `dev_default` constants.
...
- **Status:** fixes-applied (2026-06-24, branch `ssr-cohort`) — the genotyper is wired into the `ssr-call` driver and emits a real VCF (plan G→H→I→J complete); the fresh-eyes multi-agent re-review returned **Request-changes**, and the **Blocker + all 6 Majors + 9 Minors are now Applied** (8 Minors deferred as follow-ups).
- **Latest fixes-applied:** [fixes_applied_2026-06-24_ssr_call_driver.md](doc/devel/reports/reviews/fixes_applied_2026-06-24_ssr_call_driver.md) — 15 Applied (B1; M1–M6; Mi6–Mi12, Mi15), 8 Deferred (Mi1–Mi5, Mi13, Mi14, Mi16). **B1**: VCF rows now dense over the cohort (`format_vcf_record` takes `n_samples`, places present calls by `locus.present`, `./.:.:.` for absent) + partial-coverage regression test. **M1/M2**: silent `"?"` contig and `sample_chemistry` triple `unwrap_or` → loud panics on the broken-invariant path (+ `should_panic` test). **M3**: shared `DEFAULT_G0_FALLBACK_P`. **M4**: `partial_cmp().unwrap()` → `total_cmp` (NaN-safe). **M5**: new `InvalidVcfName` typed error rejecting tab/newline/`,<>` in contig/sample names (+3 tests). **M6**: filtered-locus emit test. Each fix its own commit (`1999d82`→`f3f8d04`). Gates: fmt/clippy `-D warnings`/doc clean; **1287 lib tests** (+7), only `--all-targets` failure is the pre-existing `psp_writer_perf` bench panic.
...
- **Resolved by the 2026-06-24 fix run (Applied):** B1 (dense VCF rows + test); M1–M6 (loud `"?"`/`sample_chemistry`, shared `DEFAULT_G0_FALLBACK_P`, `total_cmp`, `InvalidVcfName` validation, filtered-locus emit test); Mi6 (justify the `#[allow]`s), Mi7 (`LocusSlipFit.profile` literal), Mi8 (`level_multiplier`), Mi9 (`FrozenParams.chemistry`), Mi10 (stale comment), Mi11 (`inbreeding_f` doc), Mi12 (coercion docs), Mi15 (refit non-convergence test).
- **Open (deferred from the 2026-06-24 fix run — Minors):**
  - **Mi1** — shared read-to-nearest-allele attribution helper (3–4 copies; tie-breaks agree today; best done with the planned soft-split).
  - **Mi2** — `LocusModel` bundle for `compute_data_ll`'s 11-arg signature (the `#[allow]`s are now justified in the interim).
  - **Mi3 / Mi4 / Mi16** — per-round/per-locus/once-per-run allocation levers; **bench-gated and no SSR bench exists yet**, so deferred until one does.
  - **Mi5** — `LengthBin`/`AlleleCopies` structs vs the tuple bins (multi-site cosmetic refactor).
  - **Mi13** — validate allele bytes are ACGTN/ASCII vs `from_utf8_lossy` (a different untrusted boundary than M5; design choice).
  - **Mi14** — `QUAL=.` vs `0.0` for a variable-but-zero-confidence emitted locus (**policy choice**, `.` is valid VCF).
