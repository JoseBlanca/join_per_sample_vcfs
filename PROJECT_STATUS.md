...
- **Status:** Milestone D **D3 reviewed — M3 recovered** (2026-06-23, branch `ssr-cohort`). Milestone D complete (D1+D2 shipped @ checkpoint 2; D3 clusters samples into groups + recovers per-group divergent shapes). Fixes (loop step 5) next, then Milestone E. Milestones A+B+C shipped (checkpoint 1 signed off).
- **Milestone D (D3) review:** [ssr_call_genotyping_milestone_d_d3_2026-06-23.md](doc/devel/reports/reviews/ssr_call_genotyping_milestone_d_d3_2026-06-23.md) — **Approve-with-changes**: 0 Blocker, 0 Major, 2 Minor + Nit + 1 missing test (Mi1 reference-depth into cfg; Mi2 single-linkage chaining doc; determinism test).
