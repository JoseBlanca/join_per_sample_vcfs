...
>
> - **Last completed task (2026-07-16):** **ng step 3 typed regions — D3, the windowed walk: MILESTONE D COMPLETE, at Checkpoint D**
...
>   1867 lib tests green in **both profiles**; fmt/clippy clean; **nothing outside `src/ng/`**. **⏸ CHECKPOINT D — the plan's hard pause.** **Open, recorded, needs your call:** a bundle hull can **overlap a satellite** (D1's latent hole — `swallowed` tests only the hull's *start*, so a cluster chaining a >1 kb array to a nearby tract emits a region overlapping the Satellite one). D1 breaks its own partition invariant there; D3 panics instead (a loud assert). **No fixture reaches it**, and the fix is a spec question — what does a bundle containing a satellite *mean*? — not a code cleanup. **Next after review:** Milestone E — `TypedRegionIterator` (E1, where the walk becomes streaming for real), BED scan-wider-than-emit (E2), the multi-contig integration anchor (E3).
