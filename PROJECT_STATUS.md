...
- **Status:** Milestone D **shipped — M3 recovered** (2026-06-23, branch `ssr-cohort`). Full pre-pass complete (D1+D2 @ checkpoint 2; D3 clustering + per-group shapes). **Milestone E (genotyping completeness — F/level outer loop, allele-balance, full VCF) next** in the loop. Milestones A+B+C shipped (checkpoint 1).
