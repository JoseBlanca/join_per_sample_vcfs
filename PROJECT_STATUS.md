...
>   **Next (loop):** Step I (per-locus stutter adaptation — local `θ_locus`/rate refit, shrunk), then Step J (parallel sweep + byte-identity). Real-data calibration is the separate downstream effort.
...
- **Open:** Step **I** (per-locus stutter adaptation — `θ_locus`/rate refit, shrunk) and Step **J** (parallel sweep + byte-identity) unstarted. Calibration carry-overs: H4-Mi1 representative burn-in subset selection (positional first-cap biases chemistry **and** couples to the decision-E check); H1-Mi1 `G₀` backfill universe; filtered-record e2e test; all `dev_default` constants incl. `BURN_IN_MAX_LOCI`.
