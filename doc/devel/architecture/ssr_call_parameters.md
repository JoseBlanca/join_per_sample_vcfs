...
| **slip** | a single **stutter** event: a replication / PCR / sequencing artifact that adds or removes one or more **whole repeat units** from a read's copy of the tract, so the read's tract length differs from its true (parent) allele by an integer number of units. The signed size of that difference is the **slip `Δ`** (counted in **repeat units**, not bp; `+` = expansion, `−` = contraction), the argument of the stutter kernel `S_θ(Δ)`. A slip changes **length** (whole units) — the length-changing counterpart to `ε`, which changes **composition** (substitutions, no length change); the two stay jointly identifiable because they move in different coordinates (§2). A read with `Δ = 0` is **faithful** (it did not slip). The **stutter shape** below is the distribution of `Δ`; the **stutter level** is the total probability of `Δ ≠ 0`. |
