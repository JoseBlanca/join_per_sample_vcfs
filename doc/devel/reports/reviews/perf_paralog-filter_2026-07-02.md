...
- **L3 (per-locus scratch `Vec`s) — assessed, not applied.** After L1 removed the two carrier scratch vectors, four small per-locus allocations remain (`usable`, `allele_by_genotype`, `noncarrier_base`, `carrier_branch`; each ≤ 58 or ≤ 406 f64). L1's −31 % came from removing the memoisable `ln`s, confirming the kernel is transcendental-bound; the residual cost is the per-locus `log_add_exp` (`exp`+`ln_1p`) in the H1/H2 LSE loops, which is neither memoisable nor allocation-related. Four small warm-allocator allocs against ~79 k transcendentals/locus is structurally < 1 %, and reusing them intrudes a `&mut` scratch into the public scorer signature. Recommend **skip** unless a DHAT run shows otherwise — the remaining lever is H1 (parallelise the two passes).
