...
**Overall status:** Wave 7 (M12 phase-helper split + Mi7 boundary-pass rename + M16 SampleColumns sub-structs) complete on `cohort-within-chromosome-parallel`. Code-cleanup intent ("get the branch in good shape before comparing perf with main", 2026-05-29) is the closing trajectory; the remaining four items (Mi17, Mi18, Mi15, B5) are deferred with documented reasons in §13.
...
## 13. Deferred items (closing trajectory)

The user's 2026-05-29 instruction — "we are not going to the perf yet, first I want to have this branch in good shape… all the changes related with cleaning and correcting the code, one by one" — drove Waves 3 through 7. Four items from the Q4 "apply all structural refactors" set are now explicitly deferred with the reasons below; the branch is otherwise in the intended shape.

| Item | Reason for deferral | When to revisit |
| ---- | -------------------- | --------------- |
| **Mi17** (3 `Vec<Vec<_>>` → CSR; specifically `WorkingAllele.sources_per_sample` and `UnifyAllelesScratch.other_per_sample`) | The current write pattern is non-monotonic in sample order — `sources_per_sample[s].push(...)` is interleaved across alleles during projection, and `other_per_sample[s].extend(...)` is filled by an outer "per dropped allele" loop with `(s, sources)` enumerated inside. A direct CSR conversion needs either a loop-nest reversal (per-sample outer, allele inner) or a two-pass build that snapshots cell sizes before fill. Either path is a meaningful redesign of the allele-construction order and warrants its own design pass against the byte-identity contract. The `WorkerSlot.pre_fetched_ref_bytes` member of the same trio was already addressed by M2 (Wave 2). | After Mi15's file splits land — the loop-nest decisions become easier to reason about per-file. |
| **Mi18** (`OneBasedPos` / `OneBasedRange` / `ChromId` newtypes) | Adoption touches every `u32` carrying a position, span, or chromosome id: 148+ `chrom_id` references in `cohort_block` alone, ~34 `chunk.range` references across drivers. Per-call-site `pos.0` / `range.start.0` deref noise outweighs the type-safety value in the current code, where the static `u32` discipline has not been a source of bugs. Newtypes are still the right shape if a real mix-up bug surfaces — adopt then. | When a `chrom_id`-vs-`pos` mix-up bug lands, or when a Phase B feature lifts both into the same call signature for the first time. |
| **Mi15** (split `unify_alleles.rs` / `worker.rs` / `loader.rs`) | The three files are tightly coupled by design: `project_per_position_into_scratch` writes the `working_alleles` array that `serialize_working_to_columns` reads, and the compound-detection helpers build new `WorkingAllele` slots consumed by both. Splitting these into sibling files would mostly move private helpers into `pub(super)` re-exports — the import surface would grow without making the boundaries between phases clearer. The natural seam is along `WorkingAllele` field grouping (sources / chain anchors / cohort counts), which is a different refactor from "files by phase". | After Mi17 lands (the per-sample loop-nest decisions inform where the natural seams are). |
| **B5** (cross-driver byte-identity oracle integration test) | Substantial test-scaffolding job: build a multi-position fixture exercising all drop categories (≥3 samples, MNP, multi-allelic LH-cap site, hom-REF group, below-`min_alt_obs` site, failing-`qual_phred` site, failing-`mapq_diff_t` site), run both `drive_cohort_chunked` and `drive_cohort_pipeline` end-to-end, assert byte-equal VCF body + field-by-field counter equality. The streaming oracle is still on `main`, so the test remains feasible later — but each fix wave's per-category counter assertions, the `vcf::concat::concat_fragments` round-trip tests (`vcf/concat.rs:177–262`), and the existing knob-invariance tests in `tests/cohort_cli_integration.rs` together cover the practical regression surface for now. | Before the streaming driver is removed from `main` — at that point this is the only durable shape and the oracle disappears with the source. |

Performance comparison vs `main` (the user's next step after this branch is in shape) is unblocked. Criterion baseline `pre-fixes` was saved at Wave 1; the comparison can run any time the user wants it.
