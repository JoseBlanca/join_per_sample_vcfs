...
- **`Arc` sprinkled on every field.** A struct where most or all fields are `Arc<T>` is a smell — the usual fix is to wrap the whole struct once (`Arc<Self>`) and let callers clone that, rather than refcounting every component independently. If the fields genuinely need independent shared ownership, the type is probably doing too much and should be split. Inverse smell: a `'a` lifetime threaded through three or more types only to keep one shared piece borrowed — at that point an `Arc` around the shared piece is usually the cleaner trade (see the lifetime-vs-`Arc` rule in `idiomatic.md`).
...
- **Multiple `bool` fields on a struct.** Two or more `bool` fields on the same struct triggers a check: write the 2ⁿ truth table and ask whether every row is meaningful. If any row is "can't happen", "doesn't apply", or "meaningless when the other is false", the fields are co-dependent and the struct is hiding an enum — see the co-dependent-bool sub-rule in `idiomatic.md` (rule 13(f)) for the refactor. Suspicion is highest when (a) the bools are adjacent in the struct, (b) their doc comments reference the same flag set / spec / external bit field, or (c) one field's doc mentions the other.
