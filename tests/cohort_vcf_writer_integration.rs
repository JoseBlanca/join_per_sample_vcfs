...
use pop_var_caller::var_calling::types::{CallStats, CalledChunk, Variant};
use pop_var_caller::var_calling::vcf_writer::{
    DownstreamFilters, MapqDiffFilter, VcfWriter, WriterError,
};
...
// ---------------------------------------------------------------------------
// `VcfWriter` wrapper: the chunk_order reorder + gapless-drain + emit_or_drop
// machinery layered over `CohortVcfWriter` (review M5/M8). These exercise the
// byte-identical-for-any-worker-count mechanism that the end-to-end pipeline
// relies on but no test drove directly.
// ---------------------------------------------------------------------------

/// A biallelic A→T SNP at `pos` for a 2-sample cohort. `best` is each sample's
/// argmax genotype index (0 = hom-ref); a non-zero entry makes it a variant
/// call. Scalars are inert (mapq filtering is disabled in these tests).
...
#[test]
fn vcf_writer_emits_in_chunk_order_when_chunks_arrive_out_of_order() {
    let dir = tempdir().unwrap();
    let out = dir.path().join("out.vcf");
    ...
    .unwrap();
    // chunk_order is genomic order (0 ⇒ pos 100, 1 ⇒ 200, 2 ⇒ 300); deliver
    // them out of order. The BTreeMap reorder must still emit 100, 200, 300.
    writer
...
#[test]
fn vcf_writer_finish_errors_on_chunk_order_gap() {
    let dir = tempdir().unwrap();
    let out = dir.path().join("out.vcf");
    let mut writer = VcfWriter::new(
        metadata_two_samples(),
...
#[test]
fn vcf_writer_emit_or_drop_counts_each_bucket() {
    let dir = tempdir().unwrap();
    let out = dir.path().join("out.vcf");
    let mut writer = VcfWriter::new(
        metadata_two_samples(),
    ...
    .unwrap();
    // One chunk carrying one record per outcome (the MAPQ-diff gate is covered
    // in isolation by `record_fails_mapq_diff_t_for_test` elsewhere):
    let records = vec![
